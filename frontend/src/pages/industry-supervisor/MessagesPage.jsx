...
const MessagesPage = () => {
  ...
  const selectConversation = (conv) => {
    navigate(`/industry/messages/${conv._id}`);
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setConversations((prev) =>
        prev.map((c) =>
          c._id === conversationId ? { ...c, lastMessage: body.trim(), lastMessageAt: new Date().toISOString() } : c
        )
      );
    } catch {
      // leave message in input
    } finally {
      setSending(false);
    }
  };
          ...
          {loadingConvs ? (
            ...
            <div className="p-4">
              <EmptyState
                ...
                description="Conversations are linked to student internships. They will appear here once students are assigned."
              />
            </div>
          ) : (
            conversations.map((conv) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-4 border-b border-gray-50 transition-colors hover:bg-gray-50 ${isActive ? 'bg-blue-50' : ''}`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {conv.student && (
                        <p className="text-xs text-gray-400">Re: {conv.student.firstName} {conv.student.lastName}</p>
                      )}
        ...
        {!conversationId ? (
          ...
          <>
            ...
            <div className="p-4 border-b border-gray-100 flex items-center gap-3">
              <button
                onClick={() => navigate('/industry/messages')}
                  ...
                  <div>
                    ...
                    {activeConv.student && (
                      <p className="text-xs text-gray-400">
                        Re: {activeConv.student?.firstName} {activeConv.student?.lastName}
                      </p>
                    )}
                  </div>
                </>
            ...
            <div className="p-4 border-t border-gray-100 flex gap-3">
              <textarea
                ...
                className="flex-1 px-4 py-2.5 border border-gray-300 rounded-xl text-sm outline-none resize-none focus:ring-2 focus:ring-blue-500"
                style={{ maxHeight: '120px' }}
              />
              <button
                onClick={handleSend}
                disabled={sending || !body.trim()}
                className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 disabled:opacity-50 shrink-0"
              >
                {sending ? '...' : 'Send'}
              </button>
            </div>
