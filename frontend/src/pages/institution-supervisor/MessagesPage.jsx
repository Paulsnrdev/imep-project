...
const MessagesPage = () => {
  ...
  const getConvTitle = (conv) => {
    ...
    return conv.student ? `Re: ${conv.student.firstName} ${conv.student.lastName}` : 'Conversation';
  };
      ...
      <div className={`${conversationId ? 'hidden lg:flex' : 'flex'} w-full md:w-80 shrink-0 border-r border-gray-100 flex-col bg-white rounded-l-xl overflow-hidden`}>
        ...
        <div className="flex-1 overflow-y-auto">
          {loadingConvs ? (
            ...
            <div className="p-4">
              <EmptyState title="No conversations" description="Conversations are linked to student internships and will appear here once students are assigned." />
            </div>
          ) : (
            conversations.map((conv) => {
              ...
              return (
                <button
                  key={conv._id}
                  onClick={() => navigate(`/institution/messages/${conv._id}`)}
                  className={`w-full text-left p-4 border-b border-gray-50 transition-colors hover:bg-gray-50 ${isActive ? 'bg-purple-50' : ''}`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {conv.student && <p className="text-xs text-gray-400">Re: {conv.student.firstName} {conv.student.lastName}</p>}
                      <p className={`text-xs mt-0.5 truncate ${isUnread ? 'text-gray-700' : 'text-gray-400'}`}>
                        {conv.lastMessage || 'No messages yet'}
                      </p>
                    </div>
                    {isUnread && <div className="w-2 h-2 rounded-full bg-purple-500 shrink-0 mt-1.5" />}
                  </div>
                </button>
              );
            })
          ...
          <>
            <div className="p-4 border-b border-gray-100 flex items-center gap-3">
              <button
                ...
                className="lg:hidden text-sm text-purple-600 shrink-0 mr-1"
              >
                ← Back
              </button>
              {activeConv && (
                <>
                  ...
                  <div>
                    ...
                    {activeConv.student && (
                      <p className="text-xs text-gray-400">Re: {activeConv.student?.firstName} {activeConv.student?.lastName}</p>
                    )}
                  </div>
                </>
              )}
                  ...
                  return (
                    <div key={msg._id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                      ...
                      <div className={`max-w-xs sm:max-w-md flex flex-col gap-1 ${isMine ? 'items-end' : 'items-start'}`}>
                        ...
                        <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${isMine ? 'bg-purple-600 text-white rounded-tr-sm' : 'bg-gray-100 text-gray-800 rounded-tl-sm'}`}>
                          {msg.body}
                        </div>
            ...
            <div className="p-4 border-t border-gray-100 flex gap-3">
              <textarea
                ...
                className="flex-1 px-4 py-2.5 border border-gray-300 rounded-xl text-sm outline-none resize-none focus:ring-2 focus:ring-purple-500"
                style={{ maxHeight: '120px' }}
              />
              <button
                onClick={handleSend}
                disabled={sending || !body.trim()}
                className="px-4 py-2 bg-purple-600 text-white rounded-xl text-sm font-medium hover:bg-purple-700 disabled:opacity-50 shrink-0"
              >
                {sending ? '...' : 'Send'}
              </button>
            </div>
